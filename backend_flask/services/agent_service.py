...
# ─────────────────────────────────────────────
# 완료 시 assembled_prompt + travel_plan 생성 프롬프트
# ─────────────────────────────────────────────
COMPLETION_PROMPT = """당신은 10년 경력의 전문 여행 플래너입니다.
아래 수집된 여행 정보를 바탕으로 완성형 여행 패키지를 작성하세요.
...
[travel_plan 작성 형식]
assembled_prompt를 실행한 결과물. 아래 구조 예시를 따르세요:
...
def _call_groq(system: str, user: str, temperature: float = 0.6) -> str:
    """Groq API 호출 공통 함수"""
...
def run_agent_session(messages: list) -> dict:
    ...
    try:
        ...
        # ③ 모든 슬롯 채워짐 → LLM으로 assembled_prompt + travel_plan 생성
        slot_summary = (
        ...
        raw2 = _call_groq(
            ...
            user=f"[수집된 여행 정보]\n{slot_summary}\n\nassembled_prompt와 travel_plan을 JSON으로 반환하세요:",
            temperature=0.7,
        )
        completion = _parse_json(raw2)
        assembled = completion.get("assembled_prompt", "")
        travel_plan = completion.get("travel_plan", "")
        if travel_plan:
