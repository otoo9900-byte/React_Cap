...
def get_next_question(slots: dict, last_user_msg: str = "") -> str | None:
    ...
    # 6단계: 상세 취향
    if not slots.get("detailed_preference"):
        return "여행 시 더 구체적인 상세 취향(예: 특정 명소 방문 희망, 선호하는 분위기, 야경 감상 등)이 있으신가요? 없으시면 '없음'이라고 말씀해 주세요."

    # 7단계: 음식 취향 및 피해야 할 음식
    if not slots.get("food_preference"):
        return "혹시 특별히 선호하는 음식이나 피해야 할 음식(알레르기, 비선호 음식 등)이 있으신가요? 없으시면 '없음'이라고 말씀해 주세요."
...
사용자의 대화 기록에서 아래 7가지 슬롯 정보를 추출하세요.
...
- detailed_preference: 여행 시 더 구체적인 상세 취향 (예: "야경 촬영", "한적한 곳 선호", "박물관/미술관", "쇼핑 위주", "없음")
- food_preference: 음식 취향/선호/불호/알레르기 (예: "해산물 불호", "라멘 좋아함", "유제품 알레르기", "없음")
...
- <think> 태그, 마크다운 코드블록 없이 { 로 시작하는 순수 JSON만 반환하세요.
{
  ...
  "detailed_preference": string 또는 null,
...
# ─────────────────────────────────────────────
PLAN_GENERATION_PROMPT = """당신은 10년 차 베테랑 전문 여행 플래너입니다.
제공된 여행 정보를 바탕으로 마크다운 형식의 상세 일정을 기획해 주세요.

[작성 가이드]
1. 일자별(1일차, 2일차...) 상세 일정을 시간대별로 나누어 작성해 주세요.
   - 아침, 점심, 저녁 식사 시간대별 동선을 세부적으로 표시합니다.
   - **[매우 중요]** 반드시 수집된 목적지 구역 내부에서만 현실적인 동선을 설계하세요. 목적지와 전혀 상관없는 다른 먼 도시로 강제로 순간이동하는 일정을 절대 짜지 마세요.

2. 각 일정에 알맞은 추천 장소, 식당, 그리고 대략적인 예상 비용을 포함해 주세요.
   - 장소와 식당명은 실제로 해당 목적지에 실존하는 유명하고 구체적인 실제 이름으로만 작성하세요.
   - **[식사 메뉴 가이드]** 반드시 사용자의 음식 취향 및 피해야 할 음식 조건을 엄격하게 반영하세요.

3. **[맛집/식당 추천 필수 규칙]**
   - 식사 시간대(아침/점심/저녁)마다 반드시 **2개의 식당**을 추천해 주세요. (1순위, 2순위 형태로)
   - 각 식당마다 반드시 **2가지 메뉴**를 추천하며, 각 메뉴에 대해 아래 4가지 항목을 작성하세요:
     - ① 식당 한줄 소개: 매장의 분위기와 시그니처 매력 요약
     - ② 추천 메뉴 2가지 및 가격대: 현지인 시그니처 메뉴 1가지 + 한국인 인기 메뉴 1가지
     - ③ 맛있게 먹는 법: 각 메뉴에 대한 현지인만 아는 소스 조합 또는 먹방 꿀팁
     - ④ 이용 꿀팁: 평균 대기 시간, 예약 필수 여부 등 실질적인 팁

4. 동반자와 여행 스타일에 딱 맞는 맞춤형 '여행 꿀팁'을 마지막에 3가지 추가해 주세요.
5. 결과물은 깔끔하게 표(Table) 형식으로 보여주세요.
...
def build_assembled_prompt(slots: dict) -> str:
    ...
    detailed_pref = slots.get("detailed_preference") or "없음"
    food = slots.get("food_preference") or "없음"
    
    return f"""당신은 10년 차 베테랑 전문 여행 플래너입니다. 
아래 수집된 여행 조건을 바탕으로, 고객이 대만족할 만한 완벽한 여행 일정을 기획해 주세요.
...
def run_agent_session(messages: list) -> dict:
    ...
    try:
        ...
        # 슬롯 키 정규화 (누락된 키 → null)
        for key in ["destination", "duration", "companion", "budget", "style", "detailed_preference", "food_preference"]:
        ...
        travel_plan = _call_groq(
            ...
            user=(
                ...
                f"예산: {slots['budget']}\n"
                f"스타일: {slots['style']}\n"
                f"상세 취향: {slots['detailed_preference']}\n"
                f"음식 취향 및 피해야 할 음식: {slots['food_preference']}\n\n"
